...
**Измерение (R2.0, 2026-09-26, 13 пакетов, локальный Verdaccio, тёплые кеши; машина была под нагрузкой):** гипотеза подтвердилась наполовину.

| Фаза | мс/пакет | доля | экстраполяция на 140 пакетов, concurrency 2 |
|---|---|---|---|
| `pnpm pack` | 552 | 1.7% | 0.6 мин |
| распаковка | 22 | 0.1% | ~0 |
| статические проверки (5 процессов `node`) | 251 | 0.8% | 0.3 мин |
| ping + переписывание + повторная упаковка | 234 | 0.7% | 0.3 мин |
| старт `kb` CLI (входит в следующую строку) | 1995 | 6.2% | 2.3 мин |
| **`release clean install` целиком** (CLI + Arborist-установка + import) | **31151** | **96.7%** | **36.3 мин** |
| итого | 32210 | 100% | ~37.6 мин (холодный прогон 38.3) |

- Повторная упаковка, лишние процессы и старт CLI — не узкое место (меньше 4% вместе). **Узкое место — установка пакета в чистый consumer:** медиана 4.1 с, среднее 17.4 с; `duckdb`, `qdrant`, `agent-sdk`, `studio-app`, `sdk` — 23–49 с. Размер внутреннего замыкания важен меньше, чем сторонние и нативные зависимости. Через CLI установка заметно медленнее обычного `npm install` (медиана 12.3 с против 4.1 с).
- Измерено ~38 минут против наблюдаемых автором ~80. Разницу, вероятно, дают нагрузка на машину, таймаут 120 с с повтором и холодный uplink; это не проверено.
- Проверено на редизайне: статические проверки всех 13 tarball'ов одним процессом — 9 мс (против ~250 мс на пакет сейчас); **одна общая установка 13 tarball'ов — 50 с против 227 с у 13 отдельных** (в 4.5 раза быстрее).

**Рекомендация (ожидаемо 5–10 минут на `checks`):**
1. Использовать tarball'ы, уже созданные шагом Stage, и гонять все статические проверки одним процессом.
2. Одна общая установка платформы в один consumer из staging-реестра + один проход импортов.
3. `Arborist` как библиотека, а не запуск CLI на пакет.
4. Отдельную чистую установку по пакету оставить только для публичных самостоятельных пакетов (`sdk`, `platform-client`) и изменённых с прошлого тега (порядка 10 пакетов).

Не измерено: реальный staged-поток (релизы запрещены), общая установка на 140 пакетах (только на 13), сквозное время скрипта. Заметка: `import` `@kb-labs/mcp-app` запускает демон и зависает, поэтому проверка импорта должна пропускать app-пакеты или ограничиваться resolve.
