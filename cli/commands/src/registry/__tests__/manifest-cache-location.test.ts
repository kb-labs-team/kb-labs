...
describe('manifest cache location', () => {
  it('is under the project state directory, not the repository', () => {
    const written = manifestCachePath(project);
  ...
  it('discovery writes the cache to the state directory and leaves .kb/ free of cache files', async () => {
    ...
    expect(existsSync(manifestCachePath(project))).toBe(true);
    expect(existsSync(join(project, '.kb', 'cache'))).toBe(false);
  });

  it('never reads an in-repo cache left by an older version', () => {
    mkdirSync(join(project, '.kb', 'cache'), { recursive: true });
    writeFileSync(join(project, '.kb', 'cache', 'cli-manifests.json'), '{}');

    expect(manifestCachePath(project).startsWith(project)).toBe(false);
    expect(existsSync(manifestCachePath(project))).toBe(false);
  });
});
